...
func setupEndpointSet(
	...
	duplicatedStores := promauto.With(reg).NewCounter(prometheus.CounterOpts{
		Name: fmt.Sprintf("thanos_%s_duplicated_store_addresses_total", comp),
		Help: "The number of times a duplicated store addresses is detected from the different configs",
	})
